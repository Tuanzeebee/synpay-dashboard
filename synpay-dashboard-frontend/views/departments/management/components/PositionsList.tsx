...
import { Briefcase, Plus, Search, Edit2, Trash2, Loader2 } from 'lucide-react'
...
export default function PositionsList({ positions, language, isLoading, onAdd, onEdit, onDelete }: Props) {
  ...
  const t = {
    ...
    empty: language === 'vi' ? 'Chưa có chức vụ nào' : 'No positions found',
  ...
  return (
    <Card className="transition-colors duration-300">
      ...
      <div className="divide-y divide-slate-200 dark:divide-slate-700 max-h-[600px] overflow-y-auto">
        ...
        {/* Position items */}
        {!isLoading && filteredPositions.map((pos) => (
          <div
            key={pos.id}
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-1">{pos.name}</h3>
                {pos.createdAt && (
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {t.createdAt}: {new Date(pos.createdAt).toLocaleDateString()}
                  </p>
                )}
              </div>
